<script lang="ts">
	...
	import type { DriverStats, ReviewWithDriverName } from '$lib/api/entities';
	...
	const fetchDriverStats = async () => {
		...
		try {
			stats = await reviewsApi.getDriverStats(driverId, 10);
		} catch (err) {
	...
	const getStarArray = (rating: number) => Array.from({ length: 5 }, (_, i) => i < rating);
</script>
...
<div class="driver-reviews">
	...
	{:else if stats}
		<div class="stats-header">
				...
				<div class="rating-summary">
					{#if stats.total_reviews > 0}
						<div class="average-rating">
							...
							<div class="stars-small" aria-hidden="true">
			...
			{#if stats.group_ratings && stats.group_ratings.length > 0}
				<div class="group-ratings">
					<h3>Experience by groups</h3>
					<div class="group-list">
						{#each stats.group_ratings as group}
							<div class="group-item">
								<div>
									<p class="group-label">{group.label}</p>
									<p class="group-meta">
										{group.total_reviews} {group.total_reviews === 1 ? 'review' : 'reviews'}
									</p>
								</div>
								<div class="group-score">{group.average_rating.toFixed(1)} ★</div>
							</div>
						{/each}
					</div>
				</div>
		...
		{#if stats.recent_reviews.length > 0}
			<div class="reviews-list">
				...
				{#each stats.recent_reviews as review (review.id)}
